...
public abstract class AbstractLog<T extends Message> implements Log<T>, EntriesContext {
    ...
    private final static EntrySampler   sampler = new EntrySampler();
    private final static StackHelperApi stack   = new StackHelper().withKnown(AbstractLog.class);
    ...
    protected AbstractLog(String loggerName, Class<T> messageClass) {
    ...
    private EntryLogger logger() {
        if (logger == null && EntryLogger.isInitialized()) {
            logger = new EntryLogger(loggerName);
        }
